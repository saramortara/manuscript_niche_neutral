...
\renewcommand{\Rsquared}{R{$^2$}} %PI: acho que precisa adicionar um espaço ao comando
%\renewcommand{{$|$}}{{$|$}}
%\renewcommand{\e}{{$+$}}
%\renewcommand{\int}{{$*$}}
...
% Moreover, sets of species can be more or less ecological equivalent, and thus SADs can be a mixture of abundances of two groups of species, the core (persistent) and satellite (occasional) species \citep{Magurran2003}.
...
\citep{Swenson2012}.
%The refining of the niche concept \citep{Chase2003}, modern coexistence theory \citep{Chesson2000}, and Hubbell's proposal of neutral theory \citep{Hubbell2001} contributed to
...
In addition,
%a few multilevel model approaches
multilevel statistical modelling approaches %PI
\citep{Pollock2012, Jamil2013, Jamil2013a, Miller2019, TerBraak2019} have been developed to assess trait-environment models from community data.
% Differently from the emphasis on quantifying diversity through metrics, these models allow detecting what drives species abundance in a community. Differently from the emphasis in quantifying diversity through community-wide metrics, these models relates the abundances of each species to their traits. %PI : estava repetido, achei melhor a segunda opção abaixo, comentei esta então.
%Still, much of this trait-based work has focused on species interactions independent of SADs \citep{Swenson2012}.  Though there have been efforts to incorporate trait information in SADs \citep{Magurran2003, Supp2015} by asking what defines functional groups we can go further by asking what is the relative importance of biological traits versus stochasticity in shaping species abundances.
%PI ter 13 out 2020 09:52:49 -03: Talvez indicar que a abordagem da Magurram e estes modelos vão na direção de incorporar traits na análise de SADs e com isso reconhecer diferentes grupos funcionais e que isso por sua vez pode ajudar a investigar the relative importance of biological traits versus stochasticity in shaping species abundances.
...
%PI: tentei expressar acima o que havia pensado, e sugiro usar estas entença para abrir novo parágrafo, juntando com o que começava com "one way ..."
...
%Overcoming the dichotomy debate of niche versus neutrality, Vellend proposes that biological communities are affected by the big four high-level processes defining biological communities (i.e., selection, drift, speciation, and dispersal). In this context, niche and neutral dynamics %are under the umbrella of corresponds to selection and drift. 
%PI; mudei esta parte para o trecho abaixo, pensando em já juntar com o parágrafo que começava com "One way ..."
...
% PI: acho que aqui tb dá para juntar este parágrafo com o seguinte
...
Assuming
that SADs contain core and satellite species and these groups %seem to present
have %PI
...
% Third,
We tested as a third competing hypothesis %PI: para deixar mais claro que são hipóteses alternativas
...
season (Cfb).
%Since all mountains are part of the same mountain chain,
...
% replicates from local communities.
...
replicates of local communities under the same environmental conditions. %PI
...
precipitation.
%Although altitude represents a set of variables,
Altitude thus is a proxy of a set of environmental variables, and %PI
it is
widely %PI
...
A total of 30 sites were sampled, %10
ten %PI
in each mountain range %locality,
...
(GLMMs) \cite{Bolker2009}. %PI achei que era bom incluir alguma ref de GLMM aqui. Usei esta que já estava no aqrquivo bib 
...
% PI: Não lembro mais pq tomamos esta decisão. Vc lembra? Temos alguma
% razão mais forte para justificar 40/93? tem cara de ser algo que vão
% pegar no pé.
Then, we
...
%PI: aqui não seria melhor "we also included a random variation caused by species  in the effect of gradient" ou algo similar, para indicar que o efeito aleatório é no slope?
...
% (code available at https://github.com/saramortara/niche\_neutral).
All data analyses
...
\url{https://github.com/saramortara/niche\_neutral} %PI: talvez indicar na cover letter que podemos abrir o repo a editores e revisores.
...
%Because of the novelty of our framework, we applied our framework to communities generated mainly by selection or drift dynamics. %Achei desnecessária, sugiro tirar
...
% PI QUESTAO: fiquei com uma dúvida aqui: o que vejo na figura é que as
% barrinhas de erro de parte das espécies core são pequenas, enquanto
% que as as barras das espécies ocasionais são todas grandes e abarcam
% toda a faixa cinza. A legenda diz que as barras são SDs, e a faixa
% cinza é o IC do predito pelo modelo. Se é isso, me parece que não
% bate coma descrição acima, que parece dizer que as barras são ICs.
...
%PI: esta figura das 6 espécies ficou muito melhor!
...
\citep[][\emph{sensu}]{Vellend2014} %PI
and relating them
...
%PI QUESTAO: aqui minha outra dúvida conceitual: o termo de OLRE não é sp:site? Aqui entendi que vc diz que é sp:alt
...
succeed.
% PI: estes compostos químicos são apenas os dos fotoreceptores
% descritos a seguir ou algo a mais do que isso (p.ex., maquinaria
% fotossintética mais adaptada a pouca luz?)
...
canopy cover.
% PI: super massa isso. Mas a explicação mais simples de que folhas
% grossas dificultam fotossíntese em ambientes de pouca luz não pode
% tb ser indicada aqui? Quero dizer, para além dessas adaptações
% específicas de orientar as folhas, etc?
...
%Finally, in a historical context, we can count three main strategies PI
...
% sara 2021: isso aqui mudou, não sei se os comentários se mantém mas mantive aqui 
% PI: ainda achei bem metodológico. Acho importante ter esta seção, mas pra sintetizar conceitualemnte o que entendemos por tayloring SADs. Ou seja, basicamente construir a narrativa da figura 3, mostrando como combinamos a ideia de grupos emergentes, core/ocasional e trait-related abundances.  Aliás, onde está citada a figura 3 no texto? Não achei.
...
\citep{Mcgill2003}.
%Although we have applied our modeling approach to a specific data set, it comprises a general tool to disentangle selection and drift effects in any given community. %PI; comentei aqui pq já está na conclusão, onde acho que cai melhor
...
is also an important source of differences in abundances. %PI
% also explain differences in abundances.
% We proposed a mechanism that %explains
Our conceptual models %PI
describes
...
Authors acknowledge Fundação O Boticário para Proteção da Natureza for facilitating the data collection. SM thanks CAPES DS for financial support. Authors thanks Eduardo Santos and Luis Schiesari for insightful comments on previous versions of this manuscript. SM thanks to Brian McGill, Xiao Xiao and Sarah Supp for valuable discussions on the ideas presented in this manuscript. PIP was supported by CNPq (process 313055/2020-3) and by grants 2009/53413-5 and 2013/19250-7, São Paulo Research Foundation (FAPESP). %PI: verificar com todos mais que bolsas e projetos gostariam de incluir

\section*{Authors' Contributions} 
SM, AAO and PIP performed conceptual modeling framework, SM and PIP performed statistical analysis, MP, JP and PHL collected data and identified fern species, SM wrote the manuscript with all authors contributing substantially to the final version. %PI

\section*{Data accessibility} 
All authors have agreed in putting data used in the manuscript available on Dryad.
% PI 17 dez 2020: não vai tb disponibilizar os códigos? Daí teria que indicar nos métodos ou aqui o repo
% PI: entendi que vamos disponibilizar códigos, aí já sugeri uma sentença nos métodos
